...
@section('content')
    ...
    <form action="{{ route('timetables.update', $timetable->id) }}" method="POST">
            ...
            <div class="col-xs-6 col-sm-6 col-md-6">
                <select class="form-control" name="day_id">
                    ...
                    @foreach ($days as $id => $name)
                        <option value="{{ $id }}" {{ $timetable->day_id == $id ? 'selected' : '' }}>{{ $name }}</option>
                    @endforeach
                </select>
            </div>
        </div>
            ...
            <div class="col-xs-6 col-sm-6 col-md-6">
                <select class="form-control" name="subject_id">
                    ...
                    @foreach ($subjects as $id => $name)
                        <option value="{{ $id }}" {{ $timetable->subject_id == $id ? 'selected' : '' }}>{{ $name }}</option>
                    @endforeach
                </select>
            </div>
        </div>
            ...
            <div class="col-xs-6 col-sm-6 col-md-6">
                <select class="form-control" name="lecture_hall_id">
                    ...
                    @foreach ($halls as $id => $name)
                        <option value="{{ $id }}" {{ $timetable->lecture_hall_id == $id ? 'selected' : '' }}>{{ $name }}</option>
                    @endforeach
                </select>
            </div>
        </div>
        ...
        <div class="row">
            <div class="col-xs-6 col-sm-6 col-md-6">
                <select class="form-control" name="lecturer_group_id">
                    <option value="">-- Choose Class --</option>
                    @foreach ($lec_gp as $id => $name)
                        <option value="{{ $id }}" {{ $timetable->lecturer_group_id == $id ? 'selected' : '' }}>{{ $name }}</option>
                    @endforeach
                </select>
            </div>
        </div>
